# DEVPIRA Data Saturday Piracicaba 2026

<p align="center">
  <img src="fotos/abertura/WhatsApp Image 2026-05-23 at 09.13.08.jpeg" alt="DEVPIRA Data Saturday 2026" width="800"/>
</p>

## 📅 Sobre o Evento

O **DEVPIRA Data Saturday Piracicaba 2026** é um evento gratuito voltado para a comunidade de dados, promovido pelo grupo **DEVPIRA** em Piracicaba - SP.

Faz parte da iniciativa global [Data Saturdays](https://datasaturdays.com/), que reúne profissionais e entusiastas da plataforma de dados Microsoft e tecnologias relacionadas.

🔗 [Página oficial do evento](https://eventiza.com.br/evento/devpira-datasaturday-piracicaba-2026)

---

## 📸 Fotos

As fotos do evento estão disponíveis na pasta [`/fotos`](./fotos/).

---

## 🎤 Apresentações

As apresentações estão organizadas na pasta [`/apresentacoes`](./apresentacoes/), com uma subpasta ou link para o repositório de cada palestrante.

| Palestrante | Título da Palestra | Material |
|---|---|---|
| Felipe Correia | Impacto Real da IA nos Projetos de Dados | [📄 Slides](./apresentacoes/felipe-correia/O%20Real%20Impacto%20da%20Intelig%C3%AAncia%20Artificial%20em%20Projetos%20de%20Dados.pdf) |
| Renato Groffe & Milton Camara | Produtividade no uso de Bancos de Dados com IAs: descomplicando tarefas do dia a dia com MCP Servers | [📂 Repositório](https://github.com/renatogroffe/produtividade-mcps_sqlsaturday-piracicaba-2026) |
| Hugo Venturini | Microsoft FABRIC de Ponta a Ponta: Caso Real INMET | Em breve |
| Márcio Nizzola & Carlos Machel | Multi Agentes e Workflows no Microsoft Agent Framework | [📂 Repositório](https://github.com/NIZZOLA/data-saturday-devpira-2026) · [📄 Slides](https://github.com/NIZZOLA/data-saturday-devpira-2026/blob/main/documents/Jornada_de_Desenvolvimento_com_Agentes_de_IA_-_Data_Saturday_DevPira_2026..pptx) |
| Roberto Fonseca & Marcelo Adade | SQL Server 2025 para DBAs e Desenvolvedores | Em breve |
| Carlos H. Cantu | Entendendo o controle de concorrência via MVCC/MGA/Versioning | [📄 Slides](./apresentacoes/carlos-cantu/DevPira%20-%20Entendendo%20o%20MVCC.pdf) |

> Palestrantes: para adicionar sua apresentação, crie uma pasta com seu nome em `/apresentacoes` ou abra um PR com o link para o seu repositório.

---

## 🗂️ Estrutura do Repositório

```
DEVPIRA-Data-Saturday-2026/
├── fotos/                  # Fotos do evento
├── apresentacoes/          # Apresentações dos palestrantes
│   ├── palestrante-1/
│   ├── palestrante-2/
│   └── ...
└── README.md
```

---

## 🤝 Comunidade

- 🌐 [DEVPIRA no Meetup](https://www.meetup.com/devpira/)
- 📦 [Organização no GitHub](https://github.com/dev-pira)
- 📸 [Instagram @devpira](https://www.instagram.com/devpira/)

---

<p align="center">Feito com ❤️ pela comunidade DEVPIRA · Repositório organizado com a ajuda do <a href="https://kiro.dev">Kiro</a> 🤖</p>
